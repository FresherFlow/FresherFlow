 ...
 *
 * URL scheme: role, city and skill boards live at `/jobs/{slug}-jobs` (the
 * `-jobs` suffix disambiguates boards from job-detail slugs in the one /jobs
 * namespace); year boards stay unsuffixed at `/jobs/{year}-batch`; role×city
 * combos stay unsuffixed at `/jobs/{role}-in-{city}`. Legacy unsuffixed
 * role/city/skill URLs 308 to the suffixed form, and legacy `-batch-jobs`
 * URLs 308 back to the plain `-batch` form (see resolveLegacyBoardSlug).
 *
