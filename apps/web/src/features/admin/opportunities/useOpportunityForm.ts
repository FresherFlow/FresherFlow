...
function parseExpiryDateTime(val: string | Date) {
  ...
  const hasTime =
    /[T\s]/.test(expiresAtStr) &&
    /:/.test(expiresAtStr) &&
    !expiresAtStr.includes("T00:00:00");
...
export function useOpportunityForm(
  ...
  const [duplicateCandidates, setDuplicateCandidates] = useState<
    Array<DuplicateOpportunity & { score: number }>
  >([]);
  ...
  const [newEventType, setNewEventType] =
    useState<TimelineEvent["eventType"]>("NOTIFICATION");
  ...
  const [passoutYearMin, setPassoutYearMin] = useState("");
  const [passoutYearMax, setPassoutYearMax] = useState("");
  const [allowedAvailability, setAllowedAvailability] = useState("");
  ...
  const isGovernmentJob = type === 'GOVERNMENT';
  ...
  const [governmentRequiredDocuments, setGovernmentRequiredDocuments] =
    useState("");
  const [governmentRequiredDocumentsJson, setGovernmentRequiredDocumentsJson] =
    useState("");
  ...
  const [sourceLastCheckedAt, setSourceLastCheckedAt] = useState("");
  ...
  const [appMethod, setAppMethod] = useState<"DIRECT" | "FORM" | "ASSESSMENT">(
    "DIRECT",
  );
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        const response = (await adminApi.getOpportunities({
          q: trimmedCompany,
          limit: 25,
        })) as { opportunities: DuplicateOpportunity[] };
        ...
        const scored = opportunities
          ...
          .map((opp) => {
            const titleScore = overlapRatio(
              titleTokens,
              tokenSet(opp.title || ""),
            );
            const companyScore = overlapRatio(
              companyTokens,
              tokenSet(opp.company || ""),
            );
            const candidateDomain = extractDomain(
              opp.applyLink || opp.sourceLink,
            );
            const applyDomainScore =
              currentApplyDomain &&
              candidateDomain &&
              currentApplyDomain === candidateDomain
                ? 1
                : 0;
            const score =
        ...
        setDuplicateCandidates(
          scored as Array<DuplicateOpportunity & { score: number }>,
        );
      } catch {
        setDuplicateCandidates([]);
      } finally {
  ...
  const fetchOpportunityForEdit = useCallback(async () => {
    ...
    try {
      const data = (await adminApi.getOpportunity(opportunityId)) as {
      ...
      setSalaryPeriod((opp?.salaryPeriod as "YEARLY" | "MONTHLY") || "YEARLY");
      ...
      if (opp.governmentJobDetails) {
        ...
        setApplicationStatus(
          opp.governmentJobDetails.applicationStatus || "OPEN",
        );
        setGovernmentLevel(
          opp.governmentJobDetails.governmentLevel || "CENTRAL",
        );
        ...
        setOfficialNotificationUrl(
          opp.governmentJobDetails.officialNotificationUrl || "",
        );
        setAdvertisementNumber(
          opp.governmentJobDetails.advertisementNumber || "",
        );
        setApplicationMode(opp.governmentJobDetails.applicationMode || "");
        setNotificationIssuedDate(
          opp.governmentJobDetails.notificationIssuedDate || "",
        );
        setVacancyCount(
          opp.governmentJobDetails.vacancyCount?.toString() || "",
        );
        setVacancyBreakdownJson(
          opp.governmentJobDetails.vacancyBreakdown
            ? JSON.stringify(opp.governmentJobDetails.vacancyBreakdown, null, 2)
            : "",
        );
        setApplicationFee(opp.governmentJobDetails.applicationFee || "");
        setApplicationFeeJson(
          opp.governmentJobDetails.applicationFeeDetails
            ? JSON.stringify(
                opp.governmentJobDetails.applicationFeeDetails,
                null,
                2,
        ...
        setEligibilityDetailsJson(
          opp.governmentJobDetails.eligibilityDetails
            ? JSON.stringify(
                opp.governmentJobDetails.eligibilityDetails,
                null,
                2,
        ...
        setImportantInstructions(
          opp.governmentJobDetails.importantInstructions || "",
        );
        setApplicationStartDate(
          opp.governmentJobDetails.applicationStartDate || "",
        );
        setApplicationEndDate(
          opp.governmentJobDetails.applicationEndDate || "",
        );
        setExamDate(opp.governmentJobDetails.examDate || "");
        setExamDatesJson(
          opp.governmentJobDetails.examDates
            ? JSON.stringify(opp.governmentJobDetails.examDates, null, 2)
            : "",
        );
        setAdmitCardDate(opp.governmentJobDetails.admitCardDate || "");
        setResultDate(opp.governmentJobDetails.resultDate || "");
        setSelectionStages(
          (opp.governmentJobDetails.selectionStages || []).join(", "),
        );
        setGovernmentRequiredDocuments(
          (opp.governmentJobDetails.requiredDocuments || []).join(", "),
        );
        setGovernmentRequiredDocumentsJson(
          opp.governmentJobDetails.requiredDocumentDetails
            ? JSON.stringify(
                opp.governmentJobDetails.requiredDocumentDetails,
                null,
                2,
        ...
        setExamPatternJson(
          opp.governmentJobDetails.examPattern
            ? JSON.stringify(opp.governmentJobDetails.examPattern, null, 2)
            : "",
        );
        setSkillTestsJson(
          opp.governmentJobDetails.skillTests
            ? JSON.stringify(opp.governmentJobDetails.skillTests, null, 2)
            : "",
        );
        setExamStagesJson(
          opp.governmentJobDetails.examStages
            ? JSON.stringify(opp.governmentJobDetails.examStages, null, 2)
            : "",
        );
        setImportantDatesJson(
          opp.governmentJobDetails.importantDates
            ? JSON.stringify(opp.governmentJobDetails.importantDates, null, 2)
            : "",
        );
        setQualificationDetailsJson(
          opp.governmentJobDetails.qualificationDetails
            ? JSON.stringify(
                opp.governmentJobDetails.qualificationDetails,
                null,
                2,
        ...
        setPhysicalStandardsJson(
          opp.governmentJobDetails.physicalStandards
            ? JSON.stringify(
                opp.governmentJobDetails.physicalStandards,
                null,
                2,
        ...
        setExtraMetadataJson(
          opp.governmentJobDetails.extraMetadata
            ? JSON.stringify(opp.governmentJobDetails.extraMetadata, null, 2)
            : "",
        );
        setFeeBreakdownJson(
          opp.governmentJobDetails.feeBreakdown
            ? JSON.stringify(opp.governmentJobDetails.feeBreakdown, null, 2)
            : "",
        );
        setAgeRelaxationRulesJson(
          opp.governmentJobDetails.ageRelaxationRules
            ? JSON.stringify(
                opp.governmentJobDetails.ageRelaxationRules,
                null,
                2,
        ...
        setOfficialSourceVerified(
          opp.governmentJobDetails.officialSourceVerified || false,
        );
        setNotificationPdfUrl(
          opp.governmentJobDetails.notificationPdfUrl || "",
        );
        ...
        setCategoryVacanciesJson(
          opp.governmentJobDetails.categoryVacancies
            ? JSON.stringify(
                opp.governmentJobDetails.categoryVacancies,
                null,
                2,
        ...
        setCadreDetailsJson(
          opp.governmentJobDetails.cadreDetails
            ? JSON.stringify(opp.governmentJobDetails.cadreDetails, null, 2)
            : "",
        );
        setPostPreferencesJson(
          opp.governmentJobDetails.postPreferences
            ? JSON.stringify(opp.governmentJobDetails.postPreferences, null, 2)
            : "",
        );
        setServiceBondJson(
          opp.governmentJobDetails.serviceBond
            ? JSON.stringify(opp.governmentJobDetails.serviceBond, null, 2)
            : "",
        );
        setReservationDetailsJson(
          opp.governmentJobDetails.reservationDetails
            ? JSON.stringify(
                opp.governmentJobDetails.reservationDetails,
                null,
                2,
        ...
        setReferenceLinksJson(
          opp.governmentJobDetails.referenceLinks
            ? JSON.stringify(opp.governmentJobDetails.referenceLinks, null, 2)
            : "",
        );
        setCutOffMarksJson(
          opp.governmentJobDetails.cutOffMarks
            ? JSON.stringify(opp.governmentJobDetails.cutOffMarks, null, 2)
            : "",
        );
        ...
        setSourceLastCheckedAt(
          opp.governmentJobDetails.sourceLastCheckedAt
            ? String(opp.governmentJobDetails.sourceLastCheckedAt)
            : "",
        );
        setExtractionConfidence(
          opp.governmentJobDetails.extractionConfidence !== undefined
            ? String(opp.governmentJobDetails.extractionConfidence)
            : "",
        );
      } else {
      ...
      if (opp.applicationDetails) {
        ...
        setAppDuration(
          opp.applicationDetails.estimatedMinutes?.toString() || "",
        );
  ...
  const loadTimelineEvents = useCallback(async () => {
    ...
    try {
      const response = (await adminApi.getOpportunityEvents(opportunityId)) as {
  ...
  const handleAutoFill = async (text: string) => {
    ...
    try {
      const { parsed } = (await adminApi.parseJobText(text)) as {
        parsed: ParsedJob;
      };
      ...
      if (parsed.experienceMin !== undefined)
        setExperienceMin(String(parsed.experienceMin));
      if (parsed.experienceMax !== undefined)
        setExperienceMax(String(parsed.experienceMax));
      ...
      if (parsed.salaryPeriod) {
        const normalizedSalaryPeriod = normalizeSalaryPeriodValue(
          parsed.salaryPeriod,
        );
      ...
      const parsedEducation = normalizeEducationPayload(
        parsed.allowedDegrees,
        parsed.allowedCourses,
        parsed.allowedSpecializations,
      );
      ...
      if (parsed.allowedPassoutYears?.length)
        setPassoutYears(normalizePassoutYears(parsed.allowedPassoutYears));
      ...
      toast.error("Failed to parse text. Please fill manually.", {
        id: toastId,
      });
    } finally {
      setIsParsing(false);
  ...
  const applyJsonData = (data: Partial<ParsedJob>) => {
    ...
    if ((data as any).companyLogoUrl)
      setCompanyLogoUrl((data as any).companyLogoUrl);
    ...
    if (requiredSkillsValues.length > 0)
      setRequiredSkills(requiredSkillsValues.join(", "));
    ...
    if (data.salaryAmount !== undefined)
      setSalaryAmount(String(data.salaryAmount));
    ...
    if (data.experienceMin !== undefined)
      setExperienceMin(String(data.experienceMin));
    if (data.experienceMax !== undefined)
      setExperienceMax(String(data.experienceMax));
    ...
    if ((data as any).customSlug)
      setCustomSlug(String((data as any).customSlug));
    ...
    if (data.requiredDocuments) {
      ...
      if (requiredDocumentsValues.length > 0)
        setRequiredDocuments(requiredDocumentsValues.join(", "));
    }
    ...
    if (data.governmentJobDetails) {
      ...
      if (data.governmentJobDetails.department)
        setGovernmentDepartment(String(data.governmentJobDetails.department));
      if (data.governmentJobDetails.organization)
        setGovernmentOrganization(
          String(data.governmentJobDetails.organization),
        );
      if (data.governmentJobDetails.recruitingBody)
        setRecruitingBody(String(data.governmentJobDetails.recruitingBody));
      if (data.governmentJobDetails.applicationStatus)
        setApplicationStatus(
          String(data.governmentJobDetails.applicationStatus),
        );
      if (data.governmentJobDetails.governmentLevel)
        setGovernmentLevel(String(data.governmentJobDetails.governmentLevel));
      if (data.governmentJobDetails.vacancyNature)
        setVacancyNature(String(data.governmentJobDetails.vacancyNature));
      const categoryValues = toStringArray(
        data.governmentJobDetails.jobCategory,
      );
      if (categoryValues.length > 0) setJobCategory(categoryValues.join(", "));
      if (data.governmentJobDetails.officialWebsiteUrl)
        setOfficialWebsiteUrl(
          String(data.governmentJobDetails.officialWebsiteUrl),
        );
      if (data.governmentJobDetails.officialNotificationUrl)
        setOfficialNotificationUrl(
          String(data.governmentJobDetails.officialNotificationUrl),
        );
      if (data.governmentJobDetails.advertisementNumber)
        setAdvertisementNumber(
          String(data.governmentJobDetails.advertisementNumber),
        );
      if (data.governmentJobDetails.postName)
        setPostName(String(data.governmentJobDetails.postName));
      if (data.governmentJobDetails.applicationMode)
        setApplicationMode(String(data.governmentJobDetails.applicationMode));
      if (data.governmentJobDetails.notificationIssuedDate)
        setNotificationIssuedDate(
          String(data.governmentJobDetails.notificationIssuedDate),
        );
      if (data.governmentJobDetails.vacancyCount !== undefined)
        setVacancyCount(String(data.governmentJobDetails.vacancyCount));
      if (data.governmentJobDetails.vacancyBreakdown)
        setVacancyBreakdownJson(
          JSON.stringify(data.governmentJobDetails.vacancyBreakdown, null, 2),
        );
      if (data.governmentJobDetails.applicationFee)
        setApplicationFee(String(data.governmentJobDetails.applicationFee));
      if (data.governmentJobDetails.applicationFeeDetails)
        setApplicationFeeJson(
          JSON.stringify(
            data.governmentJobDetails.applicationFeeDetails,
            null,
            2,
          ),
        );
      if (data.governmentJobDetails.ageMin !== undefined)
        setAgeMin(String(data.governmentJobDetails.ageMin));
      if (data.governmentJobDetails.ageMax !== undefined)
        setAgeMax(String(data.governmentJobDetails.ageMax));
      if (data.governmentJobDetails.ageRelaxation)
        setAgeRelaxation(String(data.governmentJobDetails.ageRelaxation));
      if (data.governmentJobDetails.eligibilityDetails)
        setEligibilityDetailsJson(
          JSON.stringify(data.governmentJobDetails.eligibilityDetails, null, 2),
        );
      if (data.governmentJobDetails.reservationNotes)
        setReservationNotes(String(data.governmentJobDetails.reservationNotes));
      if (data.governmentJobDetails.importantInstructions)
        setImportantInstructions(
          String(data.governmentJobDetails.importantInstructions),
        );
      if (data.governmentJobDetails.applicationStartDate)
        setApplicationStartDate(
          String(data.governmentJobDetails.applicationStartDate),
        );
      if (data.governmentJobDetails.applicationEndDate)
        setApplicationEndDate(
          String(data.governmentJobDetails.applicationEndDate),
        );
      if (data.governmentJobDetails.examDate)
        setExamDate(String(data.governmentJobDetails.examDate));
      if (data.governmentJobDetails.examDates)
        setExamDatesJson(
          JSON.stringify(data.governmentJobDetails.examDates, null, 2),
        );
      if (data.governmentJobDetails.admitCardDate)
        setAdmitCardDate(String(data.governmentJobDetails.admitCardDate));
      if (data.governmentJobDetails.resultDate)
        setResultDate(String(data.governmentJobDetails.resultDate));
      const stageValues = toStringArray(
        data.governmentJobDetails.selectionStages,
      );
      if (stageValues.length > 0) setSelectionStages(stageValues.join(", "));
      const docValues = toStringArray(
        data.governmentJobDetails.requiredDocuments,
      );
      if (docValues.length > 0)
        setGovernmentRequiredDocuments(docValues.join(", "));
      if (data.governmentJobDetails.requiredDocumentDetails)
        setGovernmentRequiredDocumentsJson(
          JSON.stringify(
            data.governmentJobDetails.requiredDocumentDetails,
            null,
            2,
          ),
        );
      ...
      const examCentersList = toStringArray(
        data.governmentJobDetails.examCenters,
      );
      if (examCentersList.length > 0)
        setExamCenters(examCentersList.join(", "));
      if (data.governmentJobDetails.examPattern)
        setExamPatternJson(
          JSON.stringify(data.governmentJobDetails.examPattern, null, 2),
        );
      if ((data.governmentJobDetails as any).skillTests)
        setSkillTestsJson(
          JSON.stringify(
            (data.governmentJobDetails as any).skillTests,
            null,
            2,
          ),
        );
      if ((data.governmentJobDetails as any).examStages)
        setExamStagesJson(
          JSON.stringify(
            (data.governmentJobDetails as any).examStages,
            null,
            2,
          ),
        );
      if (data.governmentJobDetails.importantDates)
        setImportantDatesJson(
          JSON.stringify(data.governmentJobDetails.importantDates, null, 2),
        );
      if (data.governmentJobDetails.qualificationDetails)
        setQualificationDetailsJson(
          JSON.stringify(
            data.governmentJobDetails.qualificationDetails,
            null,
            2,
          ),
        );
      if (data.governmentJobDetails.physicalStandards)
        setPhysicalStandardsJson(
          JSON.stringify(data.governmentJobDetails.physicalStandards, null, 2),
        );
      if (data.governmentJobDetails.extraMetadata)
        setExtraMetadataJson(
          JSON.stringify(data.governmentJobDetails.extraMetadata, null, 2),
        );
      if (data.governmentJobDetails.feeBreakdown)
        setFeeBreakdownJson(
          JSON.stringify(data.governmentJobDetails.feeBreakdown, null, 2),
        );
      if (data.governmentJobDetails.ageRelaxationRules)
        setAgeRelaxationRulesJson(
          JSON.stringify(data.governmentJobDetails.ageRelaxationRules, null, 2),
        );
      if (data.governmentJobDetails.officialSourceVerified !== undefined)
        setOfficialSourceVerified(
          Boolean(data.governmentJobDetails.officialSourceVerified),
        );
      if (data.governmentJobDetails.notificationPdfUrl)
        setNotificationPdfUrl(
          String(data.governmentJobDetails.notificationPdfUrl),
        );
      if (data.governmentJobDetails.admitCardUrl)
        setAdmitCardUrl(String(data.governmentJobDetails.admitCardUrl));
      if (data.governmentJobDetails.resultUrl)
        setResultUrl(String(data.governmentJobDetails.resultUrl));
      if (data.governmentJobDetails.answerKeyUrl)
        setAnswerKeyUrl(String(data.governmentJobDetails.answerKeyUrl));
      if (data.governmentJobDetails.syllabusUrl)
        setSyllabusUrl(String(data.governmentJobDetails.syllabusUrl));
      if (data.governmentJobDetails.previousPapersUrl)
        setPreviousPapersUrl(
          String(data.governmentJobDetails.previousPapersUrl),
        );
      if (data.governmentJobDetails.examName)
        setExamName(String(data.governmentJobDetails.examName));
      if (data.governmentJobDetails.categoryVacancies)
        setCategoryVacanciesJson(
          JSON.stringify(data.governmentJobDetails.categoryVacancies, null, 2),
        );
      if (data.governmentJobDetails.cadreDetails)
        setCadreDetailsJson(
          JSON.stringify(data.governmentJobDetails.cadreDetails, null, 2),
        );
      if (data.governmentJobDetails.postPreferences)
        setPostPreferencesJson(
          JSON.stringify(data.governmentJobDetails.postPreferences, null, 2),
        );
      if (data.governmentJobDetails.serviceBond)
        setServiceBondJson(
          JSON.stringify(data.governmentJobDetails.serviceBond, null, 2),
        );
      if (data.governmentJobDetails.reservationDetails)
        setReservationDetailsJson(
          JSON.stringify(data.governmentJobDetails.reservationDetails, null, 2),
        );
      if (data.governmentJobDetails.referenceLinks)
        setReferenceLinksJson(
          JSON.stringify(data.governmentJobDetails.referenceLinks, null, 2),
        );
      if (data.governmentJobDetails.cutOffMarks)
        setCutOffMarksJson(
          JSON.stringify(data.governmentJobDetails.cutOffMarks, null, 2),
        );
      if (data.governmentJobDetails.basicPay !== undefined)
        setBasicPay(String(data.governmentJobDetails.basicPay));
      if (data.governmentJobDetails.payLevel)
        setPayLevel(String(data.governmentJobDetails.payLevel));
      const allowanceValues = toStringArray(
        (data.governmentJobDetails as any).allowances,
      );
      if (allowanceValues.length > 0) setAllowances(allowanceValues.join(", "));
      if (data.governmentJobDetails.sourceLastCheckedAt)
        setSourceLastCheckedAt(
          String(data.governmentJobDetails.sourceLastCheckedAt),
        );
      if (data.governmentJobDetails.extractionConfidence !== undefined)
        setExtractionConfidence(
          String(data.governmentJobDetails.extractionConfidence),
        );
    }
    ...
    if (appDetails) {
      ...
      if (appDetails.estimatedMinutes !== undefined)
        setAppDuration(String(appDetails.estimatedMinutes));
      if (appDetails.requiredItems)
        setAppRequiredItems(appDetails.requiredItems);
    }
  };
  ...
  return {
    ...
    selectionStages,
    setSelectionStages,
    governmentRequiredDocuments,
    setGovernmentRequiredDocuments,
    governmentRequiredDocumentsJson,
    setGovernmentRequiredDocumentsJson,
    examCenters,
    setExamCenters,
    examPatternJson,
    ...
    importantDatesJson,
    setImportantDatesJson,
    qualificationDetailsJson,
    setQualificationDetailsJson,
    physicalStandardsJson,
    setPhysicalStandardsJson,
    extraMetadataJson,
    setExtraMetadataJson,
    feeBreakdownJson,
    setFeeBreakdownJson,
    ageRelaxationRulesJson,
    setAgeRelaxationRulesJson,
