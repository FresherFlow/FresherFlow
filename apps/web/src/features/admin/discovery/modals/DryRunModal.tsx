        ...
        <DialogHeader>
          <DialogTitle className="text-sm font-bold flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-muted text-muted-foreground font-mono text-[10px] px-2 py-0.5 rounded border border-border/40">
                {result.ats}
              </span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1 py-2">
          {!result.jobs || result.jobs.length === 0 ? (
            <div className="p-8 text-center border border-dashed border-border rounded-xl">
              <p className="text-xs font-mono text-muted-foreground">No jobs extracted in dry run.</p>
            </div>
          ) : (
            result.jobs.map((job, idx) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {job.work_mode && (
                    <span className="bg-muted text-muted-foreground font-mono text-[10px] px-2 py-0.5 rounded border border-border/40 uppercase">
                      {job.work_mode}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-border/40 text-xs font-mono mt-1">
                  {job.apply_link ? (
                    <a
                      href={job.apply_link}
                      target="_blank"
