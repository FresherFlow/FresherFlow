...
function InstagramBrandIcon({ className }: IconProps) {
    return (
        <svg viewBox="0 0 24 24" fill="currentColor" className={className}>
            ...
            <path fill="url(#ig-grad)" d="M7.75 2h8.5A5.75 5.75 0 0 1 22 7.75v8.5A5.75 5.75 0 0 1 16.25 22h-8.5A5.75 5.75 0 0 1 2 16.25v-8.5A5.75 5.75 0 0 1 7.75 2zm0 1.8A3.95 3.95 0 0 0 3.8 7.75v8.5a3.95 3.95 0 0 0 3.95 3.95h8.5a3.95 3.95 0 0 0 3.95-3.95v-8.5a3.95 3.95 0 0 0-3.95-3.95h-8.5zM12 7.3a4.7 4.7 0 1 1 0 9.4 4.7 4.7 0 0 1 0-9.4zm0 1.8a2.9 2.9 0 1 0 0 5.8 2.9 2.9 0 0 0 0-5.8zm5.35-2.2a1.1 1.1 0 1 1 0 2.2 1.1 1.1 0 0 1 0-2.2z" />
        </svg>
    );
}

function FacebookBrandIcon({ className }: IconProps) {
    return <svg viewBox="0 0 24 24" fill="#1877F2" className={className}><path d="M13.5 22v-8h2.7l.4-3h-3.1V9.1c0-.87.25-1.46 1.5-1.46h1.6V5a19 19 0 0 0-2.3-.12c-2.28 0-3.84 1.39-3.84 3.95V11H8v3h2.4v8h3.1z" /></svg>;
}
...
export function Footer({ className }: FooterProps) {
    ...
    const popularGoals = [
        { href: '/government-jobs?category=Banking', label: 'Banking & Insurance' },
        { href: '/government-jobs?category=SSC', label: 'SSC & Railways' },
        { href: '/government-jobs?category=Teaching', label: 'Teaching Exams' },
        { href: '/government-jobs?category=Defence', label: 'Defence Exams' },
        { href: '/government-jobs?category=Engineering', label: 'Engineering Exams' },
        { href: '/government-jobs?category=State', label: 'State PSC' },
        { href: '/government-jobs?category=UPSC', label: 'UPSC' },
    ];
    ...
    return (
        <footer className={cn("bg-muted/30 border-t border-border mt-auto", className)}>
            <div className="container px-4 md:px-6 mx-auto pt-10 pb-16 md:pb-8">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10 mb-12">
                    {/* Brand Section */}
                    <div className="space-y-4 col-span-2 md:col-span-3 lg:col-span-1">
                        <Link href="/" className="flex items-center gap-2">
                            <span className="font-bold text-xl tracking-tight">Fresher<span className="text-primary">Flow</span></span>
                        </Link>
                        <p className="text-sm text-muted-foreground leading-relaxed max-w-xs">
                            The community-driven platform to discover verified off-campus opportunities and government jobs with zero redirect spam.
                        </p>
                        <div className="flex flex-wrap items-center gap-3 pt-2">
                            {socialLinks.map(({ href, label, Icon }) => (
                                <a
                                    ...
                                    aria-label={label}
                                    className="h-10 w-10 rounded-full border border-border bg-background flex items-center justify-center hover:scale-110 hover:border-border/80 transition-all shadow-sm"
                                >
                    ...
                    <div className="space-y-3">
                        <h3 className="font-bold text-sm tracking-wider uppercase text-foreground">Popular Goals</h3>
                        <ul className="space-y-2">
                            {popularGoals.map((link) => (
                                <li key={link.label}>
                    ...
                    {/* Company */}
                    <div className="space-y-3">
                        <h3 className="font-bold text-sm tracking-wider uppercase text-foreground">Company</h3>
                        <ul className="space-y-2">
                    ...
                    {/* Legal */}
                    <div className="space-y-3">
                        <h3 className="font-bold text-sm tracking-wider uppercase text-foreground">Legal &amp; Misc</h3>
                        <ul className="space-y-2">
                            {legalLinks.map((link) => (
                                <li key={link.label}>
                                    <Link href={link.href} className="text-sm text-muted-foreground hover:text-primary transition-colors">
                ...
                <div className="pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left text-xs text-muted-foreground">
                    <p>
