        ...
        <div className={cn('flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground', className)}>
            ...
            {d.venueAddress && (
                <span className="inline-flex items-center gap-1 min-w-0">
                    ...
                    <span className="truncate max-w-56">{d.landmark ? `${d.venueAddress.split(',')[0]} · ${d.landmark}` : d.venueAddress}</span>
                </span>
            )}
        </div>
    );
    ...
    return (
        <div className={cn('flex items-center gap-1.5', className)}>
            <a
                ...
                className={cn(
                    'inline-flex items-center justify-center gap-1.5 rounded-lg border border-border bg-background text-foreground font-semibold hover:bg-muted transition-colors active-press duration-150 ease-out',
