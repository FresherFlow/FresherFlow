        ...
        return (
            <div className="rounded-2xl border border-border bg-card p-8 text-center space-y-3">
                <h2 className="text-base font-bold text-foreground">
                    {result.kind === 'existing' ? 'Looks like we already have this job' : 'Submitted for review'}
                </h2>
                <p className="text-xs text-muted-foreground">
                    {result.kind === 'existing'
                        ? 'We folded your link into the existing listing.'
                        : 'Thanks — your submission is in review. It has not been published yet; it will show up in listings only after a moderator approves it.'}
                </p>
                {result.kind === 'existing' ? (
                    <Link
