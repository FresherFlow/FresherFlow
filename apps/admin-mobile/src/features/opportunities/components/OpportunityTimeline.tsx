...
export const OpportunityTimeline = ({ opportunityId, events, onEventChange }: { opportunityId: string; events: Event[]; onEventChange: () => void }) => {
    ...
    const handleAddEvent = async () => {
        setSavingEvent(true);
        try {
            await adminOpportunitiesApi.addEvent(opportunityId, { type: eventType, note: eventNote.trim() || undefined });
            setEventNote('');
            setEventType('NOTE');
        ...
        Alert.alert('Delete Event?', 'Remove this timeline entry?', [
            { text: 'Cancel', style: 'cancel' },
            {
                text: 'Delete', style: 'destructive', onPress: async () => {
                    try {
                        await adminOpportunitiesApi.deleteEvent(opportunityId, eventId);
                        onEventChange();
                    } catch (e) {
        ...
        <View style={{ padding: 16 }}>
            ...
            {events.length === 0 ? (
                ...
                <View style={styles.timeline}>
                    {events.map((ev, idx) => {
                        const eColor = eventColor(ev.type);
                        return (
                            <View key={ev.id} style={styles.timelineItem}>
                                ...
                                <View style={styles.timelineBody}>
                                    <View style={styles.timelineHeader}>
                                        <View style={[styles.eventTypeBadge, { backgroundColor: alpha(eColor, 0.1) }]}>
                                            <Text style={[styles.eventTypeText, { color: eColor }]}>
                                                {ev.type.replace(/_/g, ' ')}
                                            </Text>
                                        </View>
            ...
            <Modal visible={addingEvent} transparent animationType="slide" onRequestClose={() => setAddingEvent(false)}>
                <View style={[styles.modalOverlay, { backgroundColor: alpha(colors.text, 0.4) }]}>
                                ...
                                {EVENT_TYPES.map(t => (
                                    <TouchableOpacity key={t}
                                        style={[
                                            ...
                                            eventType === t && { backgroundColor: colors.primary, borderColor: colors.primary }
                                        ]}
                                        onPress={() => setEventType(t)}>
                                        <Text style={[
                                            styles.eventChipText, 
                                            { color: colors.textMuted },
                                            eventType === t && { color: colors.white }
                                        ]}>
                                            {t.replace(/_/g, ' ')}
                                        </Text>
                                    </TouchableOpacity>
