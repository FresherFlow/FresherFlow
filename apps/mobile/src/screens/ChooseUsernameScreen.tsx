...
export const ChooseUsernameScreen: React.FC<Props> = ({ navigation }) => {
    ...
    const handleClaim = async () => {
        if (!username || !isAvailable || isClaiming) return;

        setIsClaiming(true);
        try {
            haptic.medium();
            const res = await usernameApi.claim(username);
            if (res.success) {
                haptic.success();
                updateUser({ username: res.username });
                // Navigation will be handled by AppNavigator's conditional rendering or we can manually go Main
...
const RequirementItem = ({ label, met, theme }: { label: string; met: boolean; theme: any }) => (
