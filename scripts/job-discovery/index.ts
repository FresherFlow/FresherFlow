...
import {
  discoverAtsJobs,
  discoverAggregatorJobs,
} from "./src/pipeline/discovery.js";
...
async function run() {
  ...
  const timeoutMinutes = parseInt(
    process.env.DISCOVERY_TIMEOUT_MINUTES || "80",
    10,
  );
  ...
  try {
    ...
    const discoveryMode = process.env.DISCOVERY_MODE || "all";
    if (discoveryMode === "ats") {
      await discoverAtsJobs(state);
    } else if (discoveryMode === "aggregator") {
      await Promise.all([
    ...
    const aggJobs = state.newJobsFound.filter(
      (j) => j.sourceType === "AGGREGATOR",
    );
