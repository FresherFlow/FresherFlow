  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      if (!Array.isArray(storyIds) || storyIds.length === 0) {
        console.warn('HackerNews returned empty job stories list');
        return new JobResponseDto([]);
      }

      console.log(`HackerNews returned ${storyIds.length} job story IDs`);
      ...
      console.log(`HackerNews returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      console.error(`HackerNews scrape error: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
