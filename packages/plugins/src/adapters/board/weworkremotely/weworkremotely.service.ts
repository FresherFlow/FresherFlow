  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      if (!xml || typeof xml !== 'string') {
        console.warn('Empty or invalid RSS response from We Work Remotely');
        return new JobResponseDto([]);
      }

      const items = this.parseRssItems(xml);
      console.log(`Parsed ${items.length} items from WWR RSS feed`);
      ...
      console.log(`We Work Remotely returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      console.error(`We Work Remotely scrape error: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
