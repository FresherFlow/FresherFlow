...
export class HasJobService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    console.log(`Fetching HasJob Atom feed: ${HASJOB_FEED_URL}`);
    ...
    try {
      ...
      if (!xml || typeof xml !== 'string') {
        console.warn('Empty or invalid Atom response from HasJob');
        return new JobResponseDto([]);
      }

      const entries = this.parseAtomEntries(xml);
      console.log(`Parsed ${entries.length} entries from HasJob Atom feed`);
      ...
      console.log(`HasJob returned ${jobs.length} jobs`);
      return new JobResponseDto(jobs);
    } catch (err: any) {
      console.error(`HasJob scrape error: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
