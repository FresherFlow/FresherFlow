...
export class WellfoundService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    try {
      ...
      console.log(`Wellfound: navigating to ${url.toString()}`);
      ...
      try {
        ...
        console.error('Wellfound: failed to parse __NEXT_DATA__ JSON');
        return new JobResponseDto([]);
      }
      ...
      console.log(`Wellfound: found ${listings.length} listings`);
      ...
      for (const listing of listings) {
        ...
        try {
          ...
          console.log(`Wellfound: failed to map listing: ${err.message}`);
        }
      }
