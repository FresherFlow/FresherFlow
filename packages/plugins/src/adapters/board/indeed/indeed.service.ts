...
export class IndeedService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    while (jobList.length < resultsWanted) {
      console.log(`Fetching Indeed jobs, cursor: ${cursor ?? 'initial'}`);
      ...
      try {
        ...
        if (!data) {
          console.warn('No data in Indeed response');
          break;
        }
        ...
        for (const result of results) {
          ...
          try {
            ...
            console.warn(`Error processing Indeed job ${jobKey}: ${err.message}`);
          }
        }
