...
export class TurboHireService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for TurboHire scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve a TurboHire tenant slug from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching TurboHire jobs for tenant: ${tenant}`);
      ...
      if (items.length === 0) {
        console.log(`TurboHire tenant "${tenant}" has no open roles`);
        return new JobResponseDto([]);
      }
      ...
      for (const item of items) {
        try {
          ...
          console.warn(`Error processing TurboHire role ${this.itemId(item)}: ${err.message}`);
        }
      }

      console.log(`TurboHire total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`TurboHire scrape error for ${tenant}: ${err.message}`);
    ...
    for (let page = 1; page <= TURBOHIRE_MAX_PAGES; page++) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          console.warn(`TurboHire job list not found (HTTP ${status}) for ${tenant}`);
          break;
        }
        throw err;
      }
  ...
  private async processItem(
    ...
    try {
      ...
      if (status && status >= 400 && status < 500) {
        ...
        console.warn(`TurboHire role ${jobId} detail not found (HTTP ${status})`);
        detail = null;
      } else {
        throw err;
      }
