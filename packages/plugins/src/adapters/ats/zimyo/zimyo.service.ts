...
export class ZimyoService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Zimyo scraper');
      return new JobResponseDto([]);
    }

    const orgId = this.resolveOrg(companySlug, input.companyUrl);
    if (!orgId) {
      console.warn('Could not resolve a Zimyo org id from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching Zimyo jobs for org: ${orgId}`);
      ...
      if (items === null) {
        console.log(`Zimyo org "${orgId}" has no reachable open-roles board`);
        return new JobResponseDto([]);
      }
      if (items.length === 0) {
        console.log(`Zimyo org "${orgId}" has no open roles`);
        return new JobResponseDto([]);
      }
      ...
      for (const item of items) {
        ...
        try {
          ...
          console.warn(`Error processing Zimyo role ${item?.JOB_ID}: ${err.message}`);
        }
      }

      console.log(`Zimyo total: ${jobPosts.length} jobs for ${orgId}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Zimyo scrape error for ${orgId}: ${err.message}`);
  ...
  private async fetchJobListPage(
    ...
    try {
      ...
      if (status) {
        ...
        console.warn(`Zimyo joblist returned HTTP ${status} for org ${orgId}`);
        return { data: null, hostReachable: true };
      }
      ...
      console.warn(`Zimyo joblist fetch failed for org ${orgId}: ${err?.message ?? err}`);
  ...
  private async fetchJobDetail(
    ...
    try {
      ...
      console.warn(`Zimyo jobDetails fetch failed for ${jobId}: ${err?.message ?? err}`);
      return null;
    }
  }
  ...
  private async fetchBrandName(
    ...
    try {
      ...
      console.warn(`Zimyo orgDetails fetch failed for org ${orgId}: ${err?.message ?? err}`);
      return '';
    }
  }
