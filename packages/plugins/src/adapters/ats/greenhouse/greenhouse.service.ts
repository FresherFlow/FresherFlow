...
export class GreenhouseService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for Greenhouse scraper');
    ...
    try {
      console.log(`Fetching Greenhouse jobs for company: ${companySlug}`);
      const response = await client.get(url);
      const data: GreenhouseResponse = response.data ?? { jobs: [] };
      const jobs = data.jobs ?? [];

      console.log(`Greenhouse: found ${jobs.length} raw jobs for ${companySlug}`);
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          console.warn(`Error processing Greenhouse job ${job.id}: ${err.message}`);
        }
      }
  ...
  private async scrapeWithApi(
    ...
    while (jobPosts.length < resultsWanted) {
      ...
      console.log(`Harvest API: fetching page ${page} (per_page=${perPage})`);
      const response = await client.get<GreenhouseHarvestJob[]>(url);

      const rawJobs: GreenhouseHarvestJob[] = response.data ?? [];
      if (rawJobs.length === 0) {
        console.log('Harvest API: no more jobs available');
        break;
      }

      console.log(`Harvest API: received ${rawJobs.length} jobs on page ${page}`);
      ...
      for (const job of rawJobs) {
        ...
        try {
          ...
          console.warn(`Error processing Harvest job ${job.id}: ${err.message}`);
        }
      }
