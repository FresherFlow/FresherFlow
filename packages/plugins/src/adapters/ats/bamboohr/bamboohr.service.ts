...
export class BambooHRService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for BambooHR scraper');
    ...
    try {
      console.log(`Fetching BambooHR jobs for company: ${companySlug}`);
      const response = await client.get(url);
      const data: BambooHRResponse = response.data ?? { result: [] };
      const jobs = data.result ?? [];

      console.log(`BambooHR: found ${jobs.length} raw jobs for ${companySlug}`);
      ...
      wanted.forEach((job, index) => {
        try {
          ...
          console.warn(`Error processing BambooHR job ${job.id}: ${err.message}`);
        }
      });

      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`BambooHR scrape error for ${companySlug}: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
