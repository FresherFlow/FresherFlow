...
export class PeopleStrongService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for PeopleStrong scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve a PeopleStrong tenant slug from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching PeopleStrong jobs for tenant: ${tenant}`);
      ...
      if (jobs.length === 0) {
        console.log(`PeopleStrong tenant "${tenant}" has no open roles`);
        return new JobResponseDto([]);
      }
      ...
      console.log(`PeopleStrong total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`PeopleStrong scrape error for ${tenant}: ${err.message}`);
  ...
  private async fetchJson(
    ...
    try {
      ...
      if (status) {
        ...
        console.warn(`PeopleStrong board returned HTTP ${status} for ${tenant}`);
        return { data: null, hostReachable: true };
      }
  ...
  private async fetchHtml(
    ...
    try {
      ...
      const status = err?.response?.status;
      if (status) {
        console.warn(`PeopleStrong page returned HTTP ${status} for ${tenant}`);
        return { data: null, hostReachable: true };
      }
      console.warn(
