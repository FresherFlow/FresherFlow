...
export class RecruiteeService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for Recruitee scraper');
    ...
    try {
      console.log(`Fetching Recruitee jobs for company: ${companySlug}`);
      const response = await client.get(url);
      const data: RecruiteeResponse = response.data ?? { offers: [] };
      const offers = data.offers ?? [];

      console.log(`Recruitee: found ${offers.length} raw jobs for ${companySlug}`);
      ...
      for (const offer of offers) {
        ...
        try {
          ...
          console.warn(`Error processing Recruitee offer ${offer.id}: ${err.message}`);
        }
      }

      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Recruitee scrape error for ${companySlug}: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
