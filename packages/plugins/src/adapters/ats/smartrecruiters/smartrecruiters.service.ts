...
export class SmartRecruitersService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for SmartRecruiters scraper');
    ...
    try {
      console.log(`Fetching SmartRecruiters jobs for company: ${companySlug}`);
      ...
      console.log(`SmartRecruiters total: ${jobPosts.length} jobs for ${companySlug}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`SmartRecruiters scrape error for ${companySlug}: ${err.message}`);
