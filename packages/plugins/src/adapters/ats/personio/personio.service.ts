...
export class PersonioService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for Personio scraper');
    ...
    try {
      console.log(`Fetching Personio XML feed (.de) for: ${companySlug}`);
      const response = await client.get<string>(urlDe);
      xmlData = response.data;
    } catch {
      console.log(`Personio .de failed, trying .com for: ${companySlug}`);
      try {
        const response = await client.get<string>(urlCom);
        xmlData = response.data;
        usedDomain = 'com';
      } catch (err: any) {
        console.error(`Personio scrape error for ${companySlug}: ${err.message}`);
        return new JobResponseDto([]);
      }
    }
    ...
    try {
      ...
      console.log(`Personio: found ${positions.length} positions for ${companySlug}`);
      ...
      for (const pos of positions) {
        ...
        try {
          ...
          console.warn(`Error processing Personio position ${pos.id}: ${err.message}`);
        }
      }

      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Personio XML parse error for ${companySlug}: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
