...
export class OorwinService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    if (!input.companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Oorwin scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(input.companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve an Oorwin tenant from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      ...
      console.log(`Oorwin total: ${trimmed.length} jobs for ${companyName}`);
      return new JobResponseDto(trimmed);
    } catch (err: any) {
      console.error(`Oorwin scrape error for ${tenant}: ${err.message}`);
  ...
  private async fetchListPage(
    ...
    try {
      ...
      if (body.status === 404 || (body.status !== 1 && body.success !== 1)) {
        console.warn(`Oorwin: non-success status for tenant "${tenant}" (status=${body.status})`);
        return null;
      }
      ...
      if (status === 404 || status === 400) {
        console.warn(`Oorwin tenant not found (HTTP ${status}) for "${tenant}"`);
        return null;
      }
      throw err;
    }
  ...
  private async fetchDescriptionsAndCollect(
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      try {
        ...
        console.warn(`Error processing Oorwin job ${item.id}: ${err.message}`);
      }
    }
  }
