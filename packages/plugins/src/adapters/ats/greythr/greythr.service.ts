...
export class GreytHrService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for GreytHR scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve a GreytHR tenant slug from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching GreytHR jobs for tenant: ${tenant}`);

      const jobs = await this.fetchJobs(client, tenant);
      if (jobs == null) {
        console.log(`GreytHR tenant "${tenant}" has no reachable published-roles board`);
        return new JobResponseDto([]);
      }

      if (jobs.length === 0) {
        console.log(`GreytHR tenant "${tenant}" has no open roles`);
        return new JobResponseDto([]);
      }
      ...
      for (const item of jobs) {
        ...
        try {
          ...
          console.warn(`Error processing GreytHR role ${item?.id}: ${err.message}`);
        }
      }

      console.log(`GreytHR total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`GreytHR scrape error for ${tenant}: ${err.message}`);
  ...
  private async fetchJobs(
    ...
    try {
      ...
      if (status) {
        ...
        console.warn(`GreytHR board returned HTTP ${status} for ${tenant}`);
      } else {
        // No HTTP response → transport-level failure (DNS / refused / reset / timeout):
        // the tenant host is unreachable. Degrade gracefully.
        console.warn(`GreytHR board fetch failed for ${tenant}: ${err?.message ?? err}`);
      }
      return null;
    }
  ...
  private coerceBody(body: unknown): GreytHrPublishedJobsResponse | null {
    ...
    if (typeof body === 'string') {
      try {
        ...
        console.warn(`GreytHR published_jobs JSON parse failed: ${err?.message ?? err}`);
      }
    }
    return null;
  }
