...
export class KekaService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Keka scraper');
      return new JobResponseDto([]);
    }

    const host = this.resolveHost(companySlug, input.companyUrl);
    if (!host) {
      console.warn('Could not resolve a Keka careers host from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      ...
      for (const job of wanted) {
        try {
          ...
          console.warn(`Error processing Keka job ${job.jobId}: ${err.message}`);
        }
      }

      console.log(`Keka total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Keka scrape error for ${host}: ${err.message}`);
    ...
    for (const path of KEKA_JOBS_API_PATHS) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          ...
          console.warn(`Keka jobs feed not found (HTTP ${status}) at ${url}`);
          continue;
        }
        throw err;
      }
  ...
  private async enrichFromDetail(
    ...
    try {
      ...
      const status = err?.response?.status;
      if (status && status >= 400 && status < 500) {
        console.warn(`Keka job ${job.jobId} detail not found (HTTP ${status})`);
        return job;
      }
