...
export class DarwinboxService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Darwinbox scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve a Darwinbox tenant from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      ...
      if (!host) {
        console.warn(`Could not resolve a live Darwinbox host for "${tenant}"`);
        return new JobResponseDto([]);
      }

      console.log(`Fetching Darwinbox jobs for tenant: ${tenant} (${host})`);
      ...
      console.log(`Darwinbox total: ${trimmed.length} jobs for ${companyName}`);
      return new JobResponseDto(trimmed);
    } catch (err: any) {
      console.error(`Darwinbox scrape error for ${tenant}: ${err.message}`);
    ...
    for (const template of DARWINBOX_HOST_TEMPLATES) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          console.warn(`Darwinbox host ${host} not found for "${tenant}" (HTTP ${status})`);
          continue;
        }
        // Network / DNS error on this region — try the next host template.
        console.warn(`Darwinbox host ${host} unreachable: ${err.message}`);
      }
    }
    return null;
  }
  ...
  private async fetchJobs(
    ...
    for (const path of DARWINBOX_JOB_LIST_PATHS) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          console.warn(`Darwinbox ${path} for "${tenant}" returned HTTP ${status}`);
          continue;
        }
        throw err;
      }
    }
    console.warn(`Darwinbox: no job-list endpoint yielded jobs for "${tenant}"`);
  ...
  private collect(
    ...
    for (const job of jobs) {
      try {
        ...
        console.warn(`Error processing Darwinbox job ${job?.id}: ${err.message}`);
      }
    }
  }
