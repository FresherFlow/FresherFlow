...
export class ZwayamService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Zwayam scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve a Zwayam tenant from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching Zwayam jobs for tenant: ${tenant.slug} (host: ${tenant.host})`);
      ...
      if (items.length === 0) {
        console.log(`Zwayam tenant "${tenant.slug}" has no open roles`);
        return new JobResponseDto([]);
      }
      ...
      for (const item of items) {
        try {
          ...
          console.warn(`Error processing Zwayam role ${this.itemId(item)}: ${err.message}`);
        }
      }

      console.log(`Zwayam total: ${jobPosts.length} jobs for ${tenant.slug}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Zwayam scrape error for ${tenant.slug}: ${err.message}`);
    ...
    for (let page = 0; page < ZWAYAM_MAX_PAGES; page++) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          console.warn(`Zwayam job list not found (HTTP ${status}) for ${tenant.slug}`);
          break;
        }
        throw err;
      }
    ...
    if (!listBody) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          ...
          console.warn(`Zwayam role ${jobId} detail not found (HTTP ${status})`);
          detail = null;
        } else {
          throw err;
        }
