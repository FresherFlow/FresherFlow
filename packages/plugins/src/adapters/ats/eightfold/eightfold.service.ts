...
export class EightfoldService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Eightfold scraper');
    ...
    try {
      console.log(`Fetching Eightfold jobs for tenant: ${host} (domain=${domain})`);
        ...
        for (let i = 0; i < offsets.length; i += EIGHTFOLD_MAX_CONCURRENCY) {
          ...
          for (const result of settled) {
            if (result.status === 'fulfilled') {
              ...
              console.warn(`Eightfold page fetch failed: ${result.reason?.message ?? result.reason}`);
            }
          }
      ...
      console.log(`Eightfold total: ${trimmed.length} jobs for ${companyName}`);
      return new JobResponseDto(trimmed);
    } catch (err: any) {
      console.error(`Eightfold scrape error for ${host}: ${err.message}`);
  ...
  private collect(
    ...
    for (const position of positions) {
      try {
        ...
        console.warn(`Error processing Eightfold position ${position?.id}: ${err.message}`);
      }
    }
  }
