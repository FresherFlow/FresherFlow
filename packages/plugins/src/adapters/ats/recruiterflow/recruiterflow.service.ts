...
export class RecruiterflowService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for Recruiterflow scraper');
    ...
    try {
      console.log(`Fetching Recruiterflow jobs for company: ${companySlug}`);
      ...
      if (!Array.isArray(jobs)) {
        console.warn(`Unexpected Recruiterflow response format for ${companySlug}`);
        return new JobResponseDto([]);
      }

      console.log(`Recruiterflow: found ${jobs.length} raw jobs for ${companySlug}`);
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          console.warn(`Error processing Recruiterflow job ${job.id}: ${err.message}`);
        }
      }

      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Recruiterflow scrape error for ${companySlug}: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
