...
export class ISmartRecruitService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for iSmartRecruit scraper');
    ...
    try {
      console.log(`Fetching iSmartRecruit jobs for company: ${companySlug}`);
      const response = await client.get(url);
      let jobs: ISmartRecruitJob[] = response.data ?? [];

      if (!Array.isArray(jobs)) {
        console.warn(`Unexpected iSmartRecruit response format for ${companySlug}`);
        return new JobResponseDto([]);
      }

      console.log(`iSmartRecruit: found ${jobs.length} raw jobs for ${companySlug}`);
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          console.warn(`Error processing iSmartRecruit job ${job.jobId}: ${err.message}`);
        }
      }

      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`iSmartRecruit scrape error for ${companySlug}: ${err.message}`);
      return new JobResponseDto([]);
    }
  }
