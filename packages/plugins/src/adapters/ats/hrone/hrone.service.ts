...
export class HrOneService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for HROne scraper');
      return new JobResponseDto([]);
    }

    const resolved = this.resolveTenant(companySlug, input.companyUrl);
    if (!resolved.tenant) {
      console.warn('Could not resolve a HROne tenant slug from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching HROne jobs for tenant: ${tenant}`);
      ...
      for (let page = 1; page <= HRONE_MAX_PAGES; page++) {
        ...
        for (const item of items) {
          ...
          try {
            ...
            console.warn(`Error processing HROne role: ${err?.message ?? err}`);
          }
        }
      ...
      console.log(`HROne total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`HROne scrape error for ${tenant}: ${err?.message ?? err}`);
  ...
  private async fetchPage(
    ...
    try {
      ...
      if (status) {
        ...
        console.warn(`HROne feed returned HTTP ${status} for ${tenant}`);
      ...
      console.warn(`HROne feed fetch failed for ${tenant}: ${err?.message ?? err}`);
  ...
  private coerceBody(data: HrOneJobsResponse | string | unknown): HrOneJobsResponse | null {
    ...
    if (typeof data === 'string') {
      ...
      try {
        ...
        console.warn(`HROne feed JSON parse failed: ${err?.message ?? err}`);
      }
    }
    return null;
  }
