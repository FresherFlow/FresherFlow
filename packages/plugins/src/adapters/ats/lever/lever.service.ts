...
export class LeverService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for Lever scraper');
    ...
    try {
      console.log(`Fetching Lever jobs for company: ${companySlug}`);
      const response = await client.get(url);
      const jobs: LeverJob[] = response.data ?? [];

      if (!Array.isArray(jobs)) {
        console.warn(`Unexpected Lever response format for ${companySlug}`);
        return new JobResponseDto([]);
      }

      console.log(`Lever: found ${jobs.length} raw jobs for ${companySlug}`);
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          console.warn(`Error processing Lever job ${job.id}: ${err.message}`);
        }
      }
