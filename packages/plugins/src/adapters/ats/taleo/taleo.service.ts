...
export class TaleoService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug) {
      console.warn('No companySlug provided for Taleo scraper');
    ...
    try {
      ...
      console.log(`Taleo total: ${jobPosts.length} jobs for ${company}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Taleo scrape error for ${company}: ${err.message}`);
      return new JobResponseDto(jobPosts);
    }
  }
