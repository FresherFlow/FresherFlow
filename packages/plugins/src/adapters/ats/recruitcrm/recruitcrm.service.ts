...
export class RecruitCrmService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Recruit CRM scraper');
      return new JobResponseDto([]);
    }

    const accountSlug = this.resolveAccountSlug(companySlug, input.companyUrl);
    if (!accountSlug) {
      console.warn('Could not resolve a Recruit CRM account slug from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching Recruit CRM jobs for account: ${accountSlug}`);
      ...
      console.log(`Recruit CRM total: ${trimmed.length} jobs for ${accountSlug}`);
      return new JobResponseDto(trimmed);
    } catch (err: any) {
      console.error(`Recruit CRM scrape error for ${accountSlug}: ${err.message}`);
