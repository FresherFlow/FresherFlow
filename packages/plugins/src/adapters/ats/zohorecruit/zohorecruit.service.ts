...
export class ZohoRecruitService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Zoho Recruit scraper');
    ...
    try {
      ...
      console.log(`Fetching Zoho Recruit careers for tenant: ${host}`);
      ...
      console.log(`Zoho Recruit total: ${trimmed.length} jobs for ${companyName}`);
      return new JobResponseDto(trimmed);
    } catch (err: any) {
      console.error(`Zoho Recruit scrape error for ${host}: ${err.message}`);
  ...
  private collect(
    ...
    for (const opening of openings) {
      try {
        ...
        console.warn(`Error processing Zoho Recruit opening ${opening?.id}: ${err.message}`);
      }
    }
  }
