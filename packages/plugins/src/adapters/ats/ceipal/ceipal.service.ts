...
export class CeipalService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    if (!input.companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Ceipal scraper');
      return new JobResponseDto([]);
    }

    const apiKey = this.resolveApiKey(input.companySlug, input.companyUrl);
    if (!apiKey) {
      console.warn('Could not resolve a Ceipal career-portal API key from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching Ceipal job postings for tenant key: ${this.maskKey(apiKey)}`);

      // First page → rows + true page/count metadata for this tenant.
      const first = await this.fetchListPage(client, apiKey, 1);
      if (!first) {
        console.warn(`Ceipal tenant key not matched or no jobs: ${this.maskKey(apiKey)}`);
        return new JobResponseDto([]);
      }
      ...
      console.log(`Ceipal total: ${trimmed.length} jobs for ${companyName}`);
  ...
  private async fetchListPage(
    ...
    try {
      ...
      if (!body || typeof body !== 'object') {
        console.warn(`Ceipal: non-JSON response for key ${this.maskKey(apiKey)} page ${page}`);
        return null;
      }
      ...
      if (status === 404 || status === 400) {
        console.warn(`Ceipal job postings not found (HTTP ${status}) for ${this.maskKey(apiKey)}`);
        return null;
      }
      throw err;
    }
  ...
  private async collectRows(
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      try {
        ...
        console.warn(`Error processing Ceipal job ${this.rowId(row) ?? '?'}: ${err.message}`);
      }
    }
  }
