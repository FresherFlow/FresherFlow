...
export class PyjamaHrService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for PyjamaHR scraper');
      return new JobResponseDto([]);
    }

    const tenant = this.resolveTenant(companySlug, input.companyUrl);
    if (!tenant) {
      console.warn('Could not resolve a PyjamaHR tenant slug from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      console.log(`Fetching PyjamaHR jobs for tenant: ${tenant}`);
      ...
      if (items.length === 0) {
        console.log(`PyjamaHR tenant "${tenant}" has no open roles`);
        return new JobResponseDto([]);
      }
      ...
      for (const item of items) {
        try {
          ...
          console.warn(`Error processing PyjamaHR role ${item.id}: ${err.message}`);
        }
      }

      console.log(`PyjamaHR total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`PyjamaHR scrape error for ${tenant}: ${err.message}`);
    ...
    for (let page = 1; page <= PYJAMAHR_MAX_PAGES; page++) {
      ...
      try {
        ...
        if (status && status >= 400 && status < 500) {
          console.warn(`PyjamaHR job list not found (HTTP ${status}) for ${tenant}`);
          break;
        }
        throw err;
      }
  ...
  private async processItem(
    ...
    try {
      ...
      if (status && status >= 400 && status < 500) {
        ...
        console.warn(`PyjamaHR role ${jobId} detail not found (HTTP ${status})`);
        detail = null;
      } else {
        throw err;
      }
