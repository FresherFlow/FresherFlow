...
export class SnaphuntService implements IScraper {
  ...
  async scrape(input: ScraperInputDto): Promise<JobResponseDto> {
    ...
    if (!companySlug && !input.companyUrl) {
      console.warn('No companySlug or companyUrl provided for Snaphunt scraper');
      return new JobResponseDto([]);
    }

    const host = this.resolveHost(companySlug, input.companyUrl);
    if (!host) {
      console.warn('Could not resolve a Snaphunt career-site host from input');
      return new JobResponseDto([]);
    }
    ...
    try {
      ...
      for (const entry of wanted) {
        try {
          ...
          console.warn(`Error processing Snaphunt job ${entry.jobId}: ${err.message}`);
        }
      }

      console.log(`Snaphunt total: ${jobPosts.length} jobs for ${tenant}`);
      return new JobResponseDto(jobPosts);
    } catch (err: any) {
      console.error(`Snaphunt scrape error for ${host}: ${err.message}`);
  ...
  private async fetchSitemap(
    ...
    try {
      ...
      if (status && status >= 400 && status < 500) {
        console.warn(`Snaphunt sitemap not found (HTTP ${status}) at ${url}`);
        return [];
      }
      throw err;
    }
  ...
  private async processEntry(
    ...
    try {
      ...
      if (status && status >= 400 && status < 500) {
        ...
        console.warn(`Snaphunt job ${entry.jobId} not found (HTTP ${status})`);
        return null;
      }
      throw err;
    }
