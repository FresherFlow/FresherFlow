...
import { setCdnMetadata } from './template-parser.js';

const CDN_BASE_URL = (process.env.NEXT_PUBLIC_CDN_URL || process.env.CDN_URL || 'https://cdn.fresherflow.in').replace(/\/$/, '');
...
export async function loadCdnMetadata(): Promise<void> {
    ...
    // 6. ATS Oracle CE URL prefix mappings (ats/oracle.json → { urlPrefix: companyName })
    const oracleJson = await fetchWithCache('ats/oracle.json');
    if (oracleJson) {
        try {
