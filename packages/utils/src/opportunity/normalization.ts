// @fresherflow/utils — Opportunity Data Normalization
...
export function normalizeSalary(text: string): NormalizedSalary {
    ...
    // "5 LPA"
    const singleLpaMatch = text.match(/([\d.]+)\s*(?:lac|lacs|lpa)/i);
    if (singleLpaMatch) {
...
export const normalizeExpiry = (text: string): string | undefined => {
    ...
    for (const p of patterns) {
        ...
        const cleaned = raw.trim()
            .replace(/\b(\d+)st\b/gi, '$1')
            .replace(/\b(\d+)nd\b/gi, '$1')
            .replace(/\b(\d+)rd\b/gi, '$1')
